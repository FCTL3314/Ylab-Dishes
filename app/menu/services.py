...
class MenuService(AbstractCRUDService):
    async def retrieve(self, menu_id: UUID, session: AsyncSession) -> MenuBase:
        menu = await self.repository.get(menu_id, session)
        is_obj_exists_or_404(menu, MENU_NOT_FOUND_MESSAGE)
        return menu

    async def list(self, session: AsyncSession) -> list[MenuBase]:
        return await self.repository.all(session)

    async def create(self, menu: Menu, session: AsyncSession) -> MenuBase:
